...
export default function TokenPaywall({ profile, tokens, requiresToken, onUseToken, onBack }: TokenPaywallProps) {
  ...
  if (purchased) {
    return (
      <div className="max-w-sm mx-auto px-4 py-20 text-center space-y-4 animate-fade-in">
        ...
        <h2 className="font-black text-2xl text-white">Tokens added!</h2>
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-6 space-y-5 animate-fade-in">
      <button onClick={onBack} className="flex items-center gap-1 text-muted text-sm hover:text-white">
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 text-center space-y-3">
        ...
        <div>
          <h2 className="font-black text-white text-xl">Daily limit reached</h2>
          <p className="text-muted text-sm mt-1">
            You've already rated <span className="text-white font-semibold">{profile.full_name.split(' ')[0]}</span> today.
            Rate again with a token.
          </p>
        </div>

        {/* Token balance */}
        <div className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-black/40 border border-border">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white text-sm">Get more tokens</h3>
        </div>
        ...
        <div className="space-y-2">
          {PACKAGES.map(pack => (
            <button
              ...
              style={{
                background: pack.popular ? '#c084fc11' : '#0f0f1a',
                borderColor: pack.popular ? '#c084fc' : '#1e1e30',
